...
describe 'MyEnumerable' do
  ...
  describe '#sort_by' do
    it 'sorts the enumerator using a set of keys generated by mapping the values of the enumerator through the given block' do
      expect(@collection.sort_by { |int| int }).to eq([1, 2, 4, 5, 6])
    end

    it 'returns an enumerator if a block is not provied' do
      expect(@collection.sort_by).to be_instance_of(Enumerator)
    end
  end

  describe '#reduce' do
    it 'combines all elements of the enumerator using a binary operation defined by a sybmol' do
      expect(@collection.reduce(:+)).to eq(18)
    end

    it 'combines all elements of the enumerator using a block with an initial value defined for memo' do
      expect(@collection.reduce(5) { |memo, element| memo + element }).to eq(23)
    end

    it 'combines all elements of the enumerator using a block without an initial value defined for memo' do
      expect(@collection.reduce { |memo, element| memo + element }).to eq(18)
    end
  end
end
